...
// Page 4: Beginning of left page spread  
const page3Words = [
...
words.forEach(w => {
    // --- 防御性检查：确保 w 和它的核心属性是有效的 ---
    if (!w || typeof w.left !== 'number' || typeof w.top !== 'number') {
        console.error('发现无效或不完整的单词数据，已跳过渲染:', w);
        return; 
    }
    ...
    el.addEventListener('mouseover', updateTooltipPosition);
...
if (pageIndicator) { 
  ...
  // 五、更新页码 & 保存列表
  renderSavedLayoutsList(currentPage);
...
prevPageBtn.addEventListener('click', () => changePage(-1));
    // --- Tool Functions (mostly unchanged, but acting on current page) ---
...
function updateTooltipPosition(e) {
  ...
  document.querySelectorAll('.half').forEach(half => {
    ...
    // --- Page-aware Save/Load Logic ---

    function renderSavedLayoutsList(pageIndex) {
        ...
        if (layoutsForPage.length === 0) {
            ...
            noSavedMsg.textContent = 'No layouts saved for this page yet.';
            ...
            layoutsForPage.forEach((layout, index) => {
                ...
                layoutDiv.querySelector('.delete-layout-btn').addEventListener('click', () => {
                    layoutsForPage.splice(index, 1);
                    renderSavedLayoutsList(pageIndex);
                });
    ...
    function loadLayout(state) {
        const currentWords = Array.from(poemContainer.querySelectorAll('.word'));
        
        state.words.forEach((wordData, index) => {
            const word = currentWords.find(w => w.textContent === wordData.text && !w.dataset.loaded);
            if (word) {
                ...
                word.classList.toggle('highlighted', wordData.highlighted);
                word.dataset.loaded = true; // Mark as loaded to avoid re-using for same text
            }
        });
        currentWords.forEach(w => delete w.dataset.loaded); // Cleanup
    }
